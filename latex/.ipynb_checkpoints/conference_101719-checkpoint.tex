...
Công thức đầu ra tại thời điểm \( t \):
\[ Y_t = g(V \cdot H_t + c) \]
...
Sau mỗi bước lặp, hàm mất mát (Loss function) được sử dụng để đo lường mức độ sai lệch giữa đầu ra dự đoán và đầu ra thực tế. Công thức chung của hàm mất mát:
\[ L(y, \hat{y}) = \sum L(y_t, \hat{y}_t) \]

Trong đó:
\begin{itemize}
    \item \( y_t \) là giá trị thực tế tại thời điểm \( t \)
    \item \( \hat{y}_t \) là giá trị dự đoán tại thời điểm \( t \)
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth]{Picture/RNN/RNN_Cruide_Oil_73.png} 
    \caption{RNN Crude Oil WTI 7:3}
    \label{fig:1}
    \end{minipage}
    \hfill
    \\
    \begin{minipage}{0.3\textwidth} 
    \centering
    \includegraphics[width=0.8\textwidth]{Picture/RNN/RNN_Gasoline_Oil_73.png}
    \caption{RNN Gasoline RBOB 7:3}
    \label{fig:2}
    \end{minipage}
    \hfill
    \\
    \begin{minipage}{0.3\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth]{Picture/RNN/RNN_Heating_Oil_73.png}
    \caption{RNN Heating Oil 7:3}
    \label{fig:3}
    \end{minipage}
\end{figure}
